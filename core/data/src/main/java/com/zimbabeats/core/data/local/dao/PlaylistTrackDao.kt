...
interface PlaylistTrackDao {
    @Query("""
        ...
        INNER JOIN video_playlist_tracks pt ON t.id = pt.trackId
        WHERE pt.playlistId = :playlistId
        ORDER BY pt.position ASC
    """)
    fun getTracksInPlaylist(playlistId: Long): Flow<List<TrackEntity>>

    @Query("SELECT * FROM video_playlist_tracks WHERE playlistId = :playlistId ORDER BY position ASC")
    fun getPlaylistTracks(playlistId: Long): Flow<List<PlaylistTrackEntity>>

    @Query("SELECT * FROM video_playlist_tracks WHERE playlistId = :playlistId AND trackId = :trackId")
    suspend fun getPlaylistTrack(playlistId: Long, trackId: String): PlaylistTrackEntity?

    @Query("SELECT COUNT(*) FROM video_playlist_tracks WHERE playlistId = :playlistId")
    fun getTrackCountInPlaylist(playlistId: Long): Flow<Int>

    @Query("SELECT COUNT(*) FROM video_playlist_tracks WHERE playlistId = :playlistId")
    suspend fun getTrackCountInPlaylistSync(playlistId: Long): Int

    @Query("SELECT EXISTS(SELECT 1 FROM video_playlist_tracks WHERE playlistId = :playlistId AND trackId = :trackId)")
    ...
    @Query("DELETE FROM video_playlist_tracks WHERE playlistId = :playlistId AND trackId = :trackId")
    suspend fun deletePlaylistTrack(playlistId: Long, trackId: String)

    @Query("DELETE FROM video_playlist_tracks WHERE playlistId = :playlistId")
    suspend fun deleteAllTracksFromPlaylist(playlistId: Long)

    @Query("UPDATE video_playlist_tracks SET position = :newPosition WHERE playlistId = :playlistId AND trackId = :trackId")
    suspend fun updateTrackPosition(playlistId: Long, trackId: String, newPosition: Int)

    @Query("SELECT MAX(position) FROM video_playlist_tracks WHERE playlistId = :playlistId")
